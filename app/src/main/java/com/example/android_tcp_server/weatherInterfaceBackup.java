...
public class weatherInterfaceBackup {
...
//
  ///      }
   //     catch (IOException e){}
    ...
    public boolean readWeather(Context context, Handler handler){
        appContext=context;
        UIHandler=handler;
    ...
    public String findTag( String tag){
        ...
        if (weatherXML==null)
            return missing;
        if (weatherXML.length()<10)
            return missing;
        String startTag = "<"+tag+">";
        String endTag = "</"+tag+">";
        if(weatherXML.contains(startTag) && weatherXML.contains(endTag)){
            int startIndex= weatherXML.indexOf(startTag)+startTag.length();
            int stopIndex = weatherXML.indexOf(endTag);
            return weatherXML.substring(startIndex,stopIndex);
        }
        else{
            return missing;
        }
